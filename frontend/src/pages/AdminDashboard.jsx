...
const AdminDashboard = () => {
  ...
  const handleExportPembayaran = () => {
    if (filteredPembayaran.length > 0) {
      // Format data for Excel
      const dataToExport = filteredPembayaran.map(item => ({
        'ID Pembayaran': item.kd_pembayaran,
        'Tanggal': new Date(item.tanggal).toLocaleString('id-ID'),
        'Nama Pasien': item.nama_pasien,
        'Total Biaya': parseFloat(item.total), // keep as number for excel formatting
      ...
      XLSX.writeFile(wb, `Laporan_Pembayaran_${exportMonth}_${exportYear}.xlsx`);
      showAlert("✅ Data berhasil diexport ke Excel", 'success');
    } else {
      showAlert("ℹ️ Data tidak ditemukan pada bulan tersebut", 'warning');
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 flex dark:bg-slate-900 transition-colors duration-300 relative">
      ...
      <div className="flex-1 ml-64 p-8 dark:text-slate-100">
        ...
        {activeTab === "pembayaran" && (
          <div className="card-container dark:bg-slate-800 dark:border-slate-700">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 bg-slate-50 dark:bg-slate-700/50 p-4 rounded-xl border border-slate-200 dark:border-slate-600">
              <div className="flex items-center gap-4 mb-4 md:mb-0">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Bulan</label>
                  <select
                    className="input-field p-2 text-sm dark:bg-slate-800 dark:border-slate-600 w-32"
                    value={exportMonth}
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Tahun</label>
                  <select
                    className="input-field p-2 text-sm dark:bg-slate-800 dark:border-slate-600 w-28"
                    value={exportYear}
              ...
              <button
                onClick={handleExportPembayaran}
                className="bg-emerald-500 hover:bg-emerald-600 text-white px-5 py-2.5 rounded-lg flex items-center gap-2 transition font-semibold shadow-sm"
              >
                <CreditCard className="h-5 w-5" /> Export ke Excel
              </button>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {filteredPembayaran.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="p-6 text-center text-slate-500">Data tidak ditemukan pada bulan ini.</td>
                  </tr>
                ) : (
                  filteredPembayaran.map((p) => (
                    ...
                    >
                      ...
                      <td className="p-3 flex items-center gap-2">
                        ...
                        {p.keterangan === "Lunas" && (
                          <button
                            onClick={() => handleDeletePembayaran(p.kd_pembayaran, p.keterangan)}
