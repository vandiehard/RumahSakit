...
import { LogOut, Pill, Users, DoorOpen, Plus, Edit, X, Activity, Scissors, CreditCard, Sun, Moon } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const [resPend, resObat, resRuangan, resDokter, resPerawat, resPenyakit, resTindakan, resPembayaran] = await Promise.all([
  ...
  const submitUpdatePembayaran = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:5000/api/admin/pembayaran/${modalData.id}`, { keterangan: modalData.keterangan });
    ...
    <div className="min-h-screen bg-slate-50 flex dark:bg-slate-900 transition-colors duration-300 relative">
      ...
      <div className="w-64 bg-slate-900 text-white min-h-screen p-4 flex flex-col fixed border-r border-slate-800 z-10 overflow-y-auto">
        ...
        <nav className="flex-1 space-y-2 mb-6">
          <button onClick={() => setActiveTab('tugas')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition ${activeTab === 'tugas' ? 'bg-primary text-white' : 'text-slate-300 hover:bg-slate-800'}`}>
            <Users className="h-5 w-5" /> Distribusi Tugas
          </button>
          <button onClick={() => setActiveTab('pembayaran')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition ${activeTab === 'pembayaran' ? 'bg-primary text-white' : 'text-slate-300 hover:bg-slate-800'}`}>
            <CreditCard className="h-5 w-5" /> Pembayaran
          </button>
          <button onClick={() => setActiveTab('obat')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition ${activeTab === 'obat' ? 'bg-primary text-white' : 'text-slate-300 hover:bg-slate-800'}`}>
            <Pill className="h-5 w-5" /> Manajemen Obat
          </button>
          <button onClick={() => setActiveTab('penyakit')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition ${activeTab === 'penyakit' ? 'bg-primary text-white' : 'text-slate-300 hover:bg-slate-800'}`}>
            <Activity className="h-5 w-5" /> Penyakit
          </button>
          <button onClick={() => setActiveTab('tindakan')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition ${activeTab === 'tindakan' ? 'bg-primary text-white' : 'text-slate-300 hover:bg-slate-800'}`}>
            <Scissors className="h-5 w-5" /> Tindakan Medis
          </button>
          <button onClick={() => setActiveTab('ruangan')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition ${activeTab === 'ruangan' ? 'bg-primary text-white' : 'text-slate-300 hover:bg-slate-800'}`}>
            <DoorOpen className="h-5 w-5" /> Audit Ruangan
          </button>
        </nav>
        
        <div className="mt-auto space-y-2">
          <button onClick={toggleTheme} className="flex items-center gap-3 w-full px-4 py-3 text-slate-300 hover:bg-slate-800 rounded-lg transition">
            {isDarkMode ? <Sun className="h-5 w-5" /> : <Moon className="h-5 w-5" />} 
            {isDarkMode ? 'Tema Terang' : 'Tema Gelap'}
          </button>
          <button onClick={handleLogout} className="flex items-center gap-3 w-full px-4 py-3 text-red-400 hover:bg-red-900/30 rounded-lg transition">
              ...
              <tbody>
                {pendaftaran.map((p) => (
                  <tr key={p.kd_pendaftaran} className="border-b border-slate-200 dark:border-slate-700">
                    ...
                    <td className="p-3">
                      <button onClick={() => assignTask(p.kd_pendaftaran, document.getElementById(`dok-${p.kd_pendaftaran}`).value, document.getElementById(`per-${p.kd_pendaftaran}`).value, document.getElementById(`rua-${p.kd_pendaftaran}`).value, p.status)} className="bg-primary text-white px-3 py-1 rounded text-sm hover:bg-secondary transition">
                        Simpan
                      </button>
                    </td>
                  </tr>
        ...
        {activeTab === 'pembayaran' && (
          <div className="card-container dark:bg-slate-800 dark:border-slate-700">
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {pembayaran.map((p) => (
                  <tr key={p.kd_pembayaran} className="border-b border-slate-200 dark:border-slate-700">
                    <td className="p-3 text-sm">{new Date(p.tanggal).toLocaleString('id-ID')}</td>
                    <td className="p-3 font-medium">{p.nama_pasien}</td>
                    <td className="p-3 font-bold text-primary dark:text-blue-400">Rp {p.total.toLocaleString()}</td>
                    <td className="p-3 text-sm text-slate-500 dark:text-slate-400">
                       <span className={`px-2 py-1 rounded-full text-xs font-bold ${p.keterangan === 'Lunas' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
                         {p.keterangan}
                       </span>
                    </td>
                    <td className="p-3">
                      <button onClick={() => { setModalData({ id: p.kd_pembayaran, keterangan: p.keterangan }); setModalType('edit_pembayaran'); }} className="text-primary dark:text-blue-400 hover:text-secondary p-2 transition">
        ...
        {activeTab === 'obat' && (
          <div className="card-container dark:bg-slate-800 dark:border-slate-700">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => { setModalData({ nama_obat: '', satuan: 'Tablet', stok: 100, keterangan: '-' }); setModalType('tambah_obat'); }} className="bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded flex items-center gap-2 transition">
              ...
              <tbody>
                {obat.map((o) => (
                  <tr key={o.kd_obat} className="border-b border-slate-200 dark:border-slate-700">
                    ...
                    <td className="p-3">
                      <button onClick={() => { setModalData({ id: o.kd_obat, stok: o.stok, nama_obat: o.nama_obat }); setModalType('edit_stok'); }} className="text-primary dark:text-blue-400 hover:text-secondary p-2 transition" title="Update Stok">
                        <Edit className="h-5 w-5" />
                      </button>
                    </td>
                  </tr>
        ...
        {activeTab === 'penyakit' && (
          <div className="card-container dark:bg-slate-800 dark:border-slate-700">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => { setModalData({ nama_penyakit: '', keterangan: '' }); setModalType('tambah_penyakit'); }} className="bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded flex items-center gap-2 transition">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {penyakit.map((p) => (
                  <tr key={p.kd_penyakit} className="border-b border-slate-200 dark:border-slate-700">
                    <td className="p-3 font-medium">{p.nama_penyakit}</td>
                    <td className="p-3 text-sm text-slate-500 dark:text-slate-400">{p.keterangan}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {activeTab === 'tindakan' && (
          <div className="card-container dark:bg-slate-800 dark:border-slate-700">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => { setModalData({ nama_tindakan: '', harga: 0 }); setModalType('tambah_tindakan'); }} className="bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded flex items-center gap-2 transition">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {tindakan.map((t) => (
                  <tr key={t.kd_tindakan} className="border-b border-slate-200 dark:border-slate-700">
                    <td className="p-3 font-medium">{t.nama_tindakan}</td>
                    <td className="p-3 text-primary dark:text-blue-400 font-bold">Rp {t.harga.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {ruangan.map(r => (
              <div key={r.kd_ruangan} className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col hover:shadow-md transition">
                ...
                <button onClick={() => { setModalData({ id: r.kd_ruangan, status: r.status_ruangan, nama_ruangan: r.nama_ruangan }); setModalType('edit_ruangan'); }} className="mt-auto border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 py-2 rounded hover:bg-slate-50 dark:hover:bg-slate-700 transition">
                  Ubah Status
                </button>
              </div>
            ))}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in-up">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl p-6 w-full max-w-md relative">
            ...
            {modalType === 'tambah_obat' && (
              <form onSubmit={e => submitForm(e, 'obat', { nama_obat: modalData.nama_obat, satuan: modalData.satuan, stok: parseInt(modalData.stok), keterangan: modalData.keterangan }, 'Obat ditambahkan!')}>
                ...
                <div className="space-y-4">
                  ...
                  <input type="number" placeholder="Stok" required className="input-field dark:bg-slate-700 dark:border-slate-600 dark:text-white" value={modalData.stok} onChange={e => setModalData({...modalData, stok: e.target.value})} />
                  <textarea placeholder="Keterangan" className="input-field dark:bg-slate-700 dark:border-slate-600 dark:text-white" value={modalData.keterangan} onChange={e => setModalData({...modalData, keterangan: e.target.value})} />
            ...
            {modalType === 'tambah_penyakit' && (
              <form onSubmit={e => submitForm(e, 'penyakit', { nama_penyakit: modalData.nama_penyakit, keterangan: modalData.keterangan }, 'Penyakit ditambahkan!')}>
                <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-4">Tambah Penyakit</h3>
                <div className="space-y-4">
                  <input type="text" placeholder="Nama Penyakit" required className="input-field dark:bg-slate-700 dark:border-slate-600 dark:text-white" value={modalData.nama_penyakit} onChange={e => setModalData({...modalData, nama_penyakit: e.target.value})} />
                  <textarea placeholder="Keterangan" className="input-field dark:bg-slate-700 dark:border-slate-600 dark:text-white" value={modalData.keterangan} onChange={e => setModalData({...modalData, keterangan: e.target.value})} />
                  <button type="submit" className="btn-primary w-full">Simpan</button>
                </div>
              </form>
            ...
            {modalType === 'tambah_tindakan' && (
              <form onSubmit={e => submitForm(e, 'tindakan', { nama_tindakan: modalData.nama_tindakan, harga: modalData.harga }, 'Tindakan ditambahkan!')}>
                <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-4">Tambah Tindakan</h3>
                <div className="space-y-4">
                  <input type="text" placeholder="Nama Tindakan" required className="input-field dark:bg-slate-700 dark:border-slate-600 dark:text-white" value={modalData.nama_tindakan} onChange={e => setModalData({...modalData, nama_tindakan: e.target.value})} />
                  <input type="number" placeholder="Harga" required className="input-field dark:bg-slate-700 dark:border-slate-600 dark:text-white" value={modalData.harga} onChange={e => setModalData({...modalData, harga: e.target.value})} />
                  <button type="submit" className="btn-primary w-full">Simpan</button>
                </div>
              </form>
            )}
            ...
            {modalType === 'edit_stok' && (
              <form onSubmit={submitUpdateStok}>
                ...
                <div className="space-y-4">
                  <input type="number" required className="input-field dark:bg-slate-700 dark:border-slate-600 dark:text-white" value={modalData.stok} onChange={e => setModalData({...modalData, stok: e.target.value})} />
            ...
            {modalType === 'edit_ruangan' && (
              <form onSubmit={submitUpdateRuangan}>
                <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">Update Status Ruangan</h3>
                <p className="text-slate-500 mb-4">{modalData.nama_ruangan}</p>
                <div className="space-y-4">
                  <select className="input-field dark:bg-slate-700 dark:border-slate-600 dark:text-white" value={modalData.status} onChange={e => setModalData({...modalData, status: e.target.value})}>
            ...
            {modalType === 'edit_pembayaran' && (
              <form onSubmit={submitUpdatePembayaran}>
                <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">Update Status Pembayaran</h3>
                <div className="space-y-4 mt-4">
                  <select className="input-field dark:bg-slate-700 dark:border-slate-600 dark:text-white" value={modalData.keterangan} onChange={e => setModalData({...modalData, keterangan: e.target.value})}>
